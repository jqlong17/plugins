...
- `01-调研`：用于沉淀前期研究内容，包括本项目代码调研、外部项目调研、某个方向的前期研究，以及某次经验复盘或经验沉淀。该目录下文档同样按时间顺序编号，例如 `01-`、`02-`。
- `02-设计`：用于沉淀总体设计内容，例如架构设计、产品页面设计、数据模型设计、技术栈选型、测试设计与验收口径。
...
> 作为 `<角色>`，我希望 `<完成某个动作>`，从而 `<得到某种结果/价值>`。

## 4. 测试点拆解

先把用户故事拆成可验证的测试点，再进入 Red 阶段。

### 模块：M1

| 测试点编号 | 场景 | 输入/触发条件 | 预期结果 | 测试层级 |
| --- | --- | --- | --- | --- |
| TP-01 | 正常路径 | 待补充 | 待补充 | 单元/集成/E2E |
| TP-02 | 边界情况 | 待补充 | 待补充 | 单元/集成/E2E |
| TP-03 | 异常路径 | 待补充 | 待补充 | 单元/集成/E2E |

### Node 项目示例

场景：为一个 Node API 新增“按关键词搜索任务列表”的能力。

用户故事示例：

> 作为用户，我希望输入关键词后能返回匹配的任务列表，从而快速找到目标任务。

测试点示例：

| 测试点编号 | 场景 | 输入/触发条件 | 预期结果 | 测试层级 |
| --- | --- | --- | --- | --- |
| TP-01 | 正常搜索 | `keyword=report` | 返回包含 `report` 的任务列表 | 集成 |
| TP-02 | 空关键词 | `keyword=` | 返回 400 或明确的校验提示 | 集成 |
| TP-03 | 大小写兼容 | `keyword=REPORT` | 结果与 `report` 一致 | 单元/集成 |
...
- 先写 `tasks.search.test.ts`
- 用失败用例覆盖 `keyword=report`
- 断言响应状态为 `200`
- 断言返回数组中包含目标任务
- 再补一条 `keyword=` 的失败测试，断言返回 `400`
...
- 先在 `tasks.service.ts` 中实现最小搜索逻辑
- 先不做分页、排序、缓存
- 先保证 `report` 查询和空关键词校验通过
- 如果接口层已通过，先不要提前抽象复杂搜索 DSL
...
- 把搜索条件校验提炼到独立函数
- 统一 service 和 route 层的命名
- 保持 `Vitest/Jest + Supertest` 相关测试继续通过
- 记录“后续再补分页和排序”

## 5. 每轮执行记录

建议每完成一个小闭环就补一行，便于快速迭代。

| 轮次 | 对应测试点 | 当前阶段 | 结果 | 备注 |
| --- | --- | --- | --- | --- |
| Round 1 | TP-01 | Red/Green/Refactor | 待补充 | 待补充 |

## 6. 轻量验证闭环

每完成一个模块或一个可演示的小功能后，执行一次轻量验证，不追求“大而全”，优先保证快速迭代。

### 6.1 最小检查项

1. 功能是否按当前用户故事工作
2. 对应测试是否已运行并通过
3. 是否存在明显的 lint / type / build 阻断
4. 是否记录了本轮测试命令和结果
5. 是否有需要立即修复的高风险问题
...
3. 下阶段补齐计划：
""",
    "workflow/04-测试/04-轻量验证闭环.md": """# 04-轻量验证闭环

这个模板用于中小型项目的快速验证，不追求一次性做完整的大型质量门禁，而是强调：
...
1. 完成一个小功能后
2. 修完一个 bug 后
3. 完成一轮 TDD 的 Red -> Green -> Refactor 后
4. 提交前做一次快速自检时
...
| 检查项 | 是否必须 | 结果 | 说明 |
| --- | --- | --- | --- |
| 当前功能是否符合用户故事 | 必须 | 通过/失败 | 待补充 |
| 本轮新增或修改测试是否通过 | 必须 | 通过/失败 | 待补充 |
| 关键命令是否可复现 | 必须 | 通过/失败 | 待补充 |
| 构建是否通过 | 按需 | 通过/失败/不适用 | 待补充 |
| 类型检查是否通过 | 按需 | 通过/失败/不适用 | 待补充 |
| lint 是否通过 | 按需 | 通过/失败/不适用 | 待补充 |
| 是否存在明显高风险问题 | 必须 | 有/无 | 待补充 |
...
| 编号 | 问题描述 | 严重级别 | 是否已处理 | 备注 |
| --- | --- | --- | --- | --- |
| ISSUE-01 | 待补充 | 高/中/低 | 是/否 | 待补充 |
""",
